...
async def run_generate_download_file(
    ...
    try:
        ...
        if input_type == "image" or (input_type == "msrp" and not msrp_target):
            file_generation_endpoint = f"https://icon5-8081.iconluxury.today/generate-download-file/?file_id={file_id}"
        ...
        if msrp_target is None and input_type != "image":
            run_scrape_job = f"https://scraper-prod.iconluxury.today/api/v7/restart-job/{file_id}"
            async with httpx.AsyncClient(timeout=300.0) as client:
                response = await client.post(run_scrape_job, headers={"accept": "application/json"})
                response.raise_for_status()
                run_scrape_job_data = response.json()
            
            parent_logger.info(f"{log_prefix} Response from file generation service: {run_scrape_job_data}")
...
num_workers_hint: int = Query(500, ge=1, le=1000),
):
